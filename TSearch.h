...
enum TReproductionMode {HILL_CLIMBING, GENETIC_ALGORITHM, CO_GENETIC_ALGORITHM}; // Supported reproduction modes
...
class TSearch {
    ...
    void SetRandomSeed(long seed) {rs.SetRandomSeed(seed);};
		...
		void SetBO(TVector<double> &NewBO);
		...
		void SetCoEvaluationFunction(double (*CoEvalFn)(TVector<double> &v, RandomState &rs, TVector<double> &BO))
			{CoEvaluationFunction = CoEvalFn;};
		...
		TVector<double> &BestOther(void) {return BO;};
		...
		void ExecuteCoSearch(TVector<double> &BO);
	...
	private:
		...
		void DoCoSearch(int ResumeFlag, TVector<double> &BO);
    ...
    friend void *EvaluatePopulationRange(void *arg);
		//
		double CoEvaluateVector(TVector<double> &Vector, RandomState &rs, TVector<double> &BO);
	friend void *CoEvaluatePopulationRange(void *arg);
		void EvaluatePopulation(int start = 1);
		void CoEvaluatePopulation(int start, TVector<double> &BO);
		...
		void CoReproducePopulationGeneticAlgorithm(TVector<double> &BO);
		...
		void CoReproducePopulation(TVector<double> &BO);
	...
	TVector<double> BO;
		...
		double (*CoEvaluationFunction)(TVector<double> &v, RandomState &rs, TVector<double> &BO);
