...
double Prey::MapBreathingRate(double neuronOutput){
    double minRate = 0.5;
    double maxRate = 2.0;
...
// Sniff Sense 
void Prey::Sense(TVector<double> &food_scent, TVector<double> &pred_scent, double timestep)
{
    // Get Breathing Rate
    ...
    if (phase > 0) {
        ...
        for (int i = 0; i <= s_width; i++){
            ...
            fc_left +=(-2*exp((-i * i) / (2 * s_width * s_width))) * food_scent[l_loc];
            pc_left +=(-2*exp((-i * i) / (2 * s_width * s_width))) * pred_scent[l_loc];
        }
        // Right side
        ...
        for (int i = 0; i <= s_width; i++){
            ...
            fc_right +=(2*exp((-i * i) / (2 * s_width * s_width))) * food_scent[r_loc];
            pc_right +=(2*exp((-i * i) / (2 * s_width * s_width))) * pred_scent[r_loc];
        }
        // Augment depending on BR/phase
        fc_left = fc_left * phase;
        pc_left = pc_left * phase;
        fc_right = fc_right * phase;
        pc_right = pc_right * phase;
        f_sensor = fc_left + fc_right;
        p_sensor = pc_left + pc_right;
    }
...
// Update State if the agent passed food
    ...
    // Scale metabolic loss by breathing rate
